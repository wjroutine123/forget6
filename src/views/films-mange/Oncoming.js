...
export default class Oncoming extends Component {
      state = {
          dataSource:[]
      }
      columns = [
    ...
    handleDel = (id)=> {
        ...
        axios.delete(`http://localhost:8000/gateway/${id}`)
    }
    handleUpdate = (id)=>{
        this.props.history.push(`/films-mange/gateupdate/${id}`)
    }
    render() {
        return (
            <div>
                <Button type="primary" onClick={()=>{
                    this.props.history.push('/films-mange/gatecreate')
                }}>添加信息</Button>
