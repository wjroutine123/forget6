...
class DetailCreate extends Component {
    ...
    handleSubmit () {
        axios.post('http://localhost:8000/detailway',{
            ...this.state.filminfo
        }).then(res => {
            this.props.history.push('/films-mange/moviedetail')
        })
    }
    render() {
        return (
                ...
                <div style={{ marginTop: '50px' }}>
                    ...
                    <div className={this.state.current === 1 ? '' : css.active}>
                        <Form {...this.layout} ref={this.twoForm} name="basictwo">
                            ...
                            <Form.Item label="工作性质" name="role" 
                            rules={[{ required: true, message: '请输入内容!' }]}
                            >
                                <Input />
                            </Form.Item>
                        ...
                        <Button onClick={()=>{this.handleUploadPhoto()}}>上传剧照</Button>
...
export default withShop(DetailCreate)
